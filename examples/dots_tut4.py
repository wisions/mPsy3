...
class DataFile:
    def __init__(self,fname):
        self.log = codecs.open(fname,'w','utf8')  # a general log, timestamps of stimulus onsets, reponses ...
    def __call__(self,event,time,trial,args):
        if event == 'TRIAL' and trial.name == 'Lattices':
            ...
            p = self.trial_params
            print('%0.2f,%0.1f,%0.1f,%0.1f'%(p.r,degs(p.gamma),degs(p.theta),degs(trial.stimuli[i].params.th)),file=self.log)
        elif event == 'KEY' and trial.name == 'Response':
            # find the selected response pill
            if args[1] not in keys_response: return True
